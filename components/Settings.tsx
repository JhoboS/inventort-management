...
-- Ensure all columns exist in scrapped_items
...
-- REGIONAL RLS POLICIES

-- Warehouses Policies
drop policy if exists "Allow read access for all users" on warehouses;
create policy "Allow read access for all users" on warehouses for select using (true);

drop policy if exists "Allow write access for super admin" on warehouses;
create policy "Allow write access for super admin" on warehouses for all using (is_super_admin());

-- App Users Policies
drop policy if exists "Allow users to read their own profile" on app_users;
create policy "Allow users to read their own profile" on app_users for select using (auth.uid() = id or is_super_admin());

drop policy if exists "Allow users to insert their own profile" on app_users;
create policy "Allow users to insert their own profile" on app_users for insert with check (auth.uid() = id or is_super_admin());

drop policy if exists "Allow super admin to manage profiles" on app_users;
create policy "Allow super admin to manage profiles" on app_users for all using (is_super_admin());
...
-- 5. SEED DATA, RE-ESTABLISH RELATIONSHIPS & BACKFILL
...
-- Backfill existing rows with NULL warehouse_id to the default US Warehouse
...
-- Ensure users have access to the default US Warehouse if their list is empty
update app_users set assigned_warehouses = array['00000000-0000-0000-0000-000000000000'::uuid] 
where assigned_warehouses is null or assigned_warehouses = '{}';
